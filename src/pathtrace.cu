...
void pathtraceInit(Scene *scene) {
  ...
  for (Geom &g : hst_scene->geoms) {
    if (g.mesh_object.num_triangles != 0) {
      cudaMalloc(&g.mesh_object.dev_triangles,
                 g.mesh_object.num_triangles * sizeof(Triangle));
      checkCUDAError("pathtraceInit");
      cudaMemcpy(g.mesh_object.dev_triangles, g.mesh_object.triangles,
                 g.mesh_object.num_triangles * sizeof(Triangle),
  ...
  // cache for first intersections
  cudaMalloc(&dev_intersections_cache,
             pixelcount * sizeof(ShadeableIntersection));
  cudaMemset(dev_intersections_cache, 0,
             pixelcount * sizeof(ShadeableIntersection));
...
void pathtraceFree() {
  ...
  if (hst_scene) {
    for (Geom g : hst_scene->geoms) {
      if (g.mesh_object.num_triangles != 0) {
        cudaFree(g.mesh_object.dev_triangles);
        //delete g.mesh_object.triangles;
      }
    }
  }
...
__global__ void computeIntersections(int depth, int num_paths,
  ...
  if (path_index < num_paths) {
    ...
    for (int i = 0; i < geoms_size; i++) {
      ...
      else if (geom.type == MESH_OBJECT) {
        t = meshObjectIntersectionTest(geom, pathSegment.ray, tmp_intersect,
                                       tmp_normal, outside);
      }
  ...
  if (idx < num_paths && pathSegments[idx].remainingBounces) {
    ...
    if (intersection.t > 0.0f) {
      ...
      // like what you would expect from shading in a rasterizer like OpenGL.
        ...
        const auto point_on_ray =
            getPointOnRay(pathSegments[idx].ray, intersection.t);

        scatterRay(pathSegments[idx], point_on_ray, intersection.surfaceNormal,
                   material, rng);

        // we have one less bounce
      ...
      // this path is done, since it is out of bounds
      pathSegments[idx].remainingBounces = 0;
    }
  }
}
...
struct HostDeviceSteamCompactionCallback {
  __host__ __device__ bool operator()(const PathSegment &p) {
    return p.remainingBounces;
  };
};

struct HostDeviceSortByMaterialCallback {
  __host__ __device__ bool
  operator()(const ShadeableIntersection &intersection1,
             const ShadeableIntersection &intersection2) {
...
void pathtrace(uchar4 *pbo, int frame, int iter) {
...
#if defined(CACHE_FIRST_INTERSECTIONS) && !defined(ANTIALIASING)
  // tracing
  dim3 numblocksPathSegmentTracing =
      (cur_paths + blockSize1d - 1) / blockSize1d;
  computeIntersections<<<numblocksPathSegmentTracing, blockSize1d>>>(
      depth, cur_paths, dev_paths, dev_geoms, hst_scene->geoms.size(),
  ...
  for (int i = 0; i < hst_scene->geoms.size(); i++)
  {
    ...
    cudaMemcpy(&dev_geoms[i], &geom, sizeof(Geom), cudaMemcpyHostToDevice);
  }
#endif

#if defined(CACHE_FIRST_INTERSECTIONS) && !defined(ANTIALIASING)
    // if at first depth and we have cache intersections, then just use cached
    // result
    if (!depth) {
    ...
    // sort by material (slow?)
    thrust::device_ptr<ShadeableIntersection> dev_intersections_ptr =
        thrust::device_pointer_cast(dev_intersections);
    thrust::device_ptr<ShadeableIntersection> dev_intersections_ptr_end =
        thrust::device_pointer_cast(dev_intersections + cur_paths);
    thrust::device_ptr<PathSegment> dev_paths_ptr =
        thrust::device_pointer_cast(dev_paths);
    thrust::sort_by_key(dev_intersections_ptr, dev_intersections_ptr_end,
                        dev_paths_ptr, HostDeviceSortByMaterialCallback());
#endif
    // TODO:
    // --- Shading Stage ---
    ...
    shadeMaterial<<<numblocksPathSegmentTracing, blockSize1d>>>(
        iter, cur_paths, dev_intersections, dev_paths, dev_materials);

    checkCUDAError("shadeMaterial");
    cudaDeviceSynchronize();

    thrust::device_ptr<PathSegment> dev_paths_ptr_start =
        thrust::device_pointer_cast(dev_paths);
    thrust::device_ptr<PathSegment> dev_paths_ptr_end =
        thrust::device_pointer_cast(dev_path_end);
    thrust::device_ptr<PathSegment> new_dev_path_end =
        thrust::partition(dev_paths_ptr_start, dev_paths_ptr_end,
                          HostDeviceSteamCompactionCallback());

    dev_path_end = thrust::raw_pointer_cast(new_dev_path_end);
