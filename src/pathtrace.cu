...
void pathtraceInit(Scene *scene) {
	...
	cudaMalloc(&dev_triangles, scene->tris.size() * sizeof(Triangle));
	cudaMemcpy(dev_triangles, scene->tris.data(), scene->tris.size() * sizeof(Triangle), cudaMemcpyHostToDevice);
...
__device__ glm::vec2 ConcentricSampleDisk(glm::vec2 u) {
...
__global__ void generateRayFromCamera(Camera cam, int iter, int traceDepth, PathSegment* pathSegments) {
	...
	if (x < cam.resolution.x && y < cam.resolution.y) {
		...
		segment.ray.direction = glm::normalize(cam.view
			- cam.right * cam.pixelLength.x * ((float)x + u0505(rng) - (float)cam.resolution.x * 0.5f)
			- cam.up * cam.pixelLength.y * ((float)y + u0505(rng) - (float)cam.resolution.y * 0.5f)
		);
#else
		segment.ray.direction = glm::normalize(cam.view
		...
		if (cam.lensRadius > 0) {
			...
			float ft = cam.focalLength / segment.ray.direction.z;
			glm::vec3 pFocus = segment.ray.origin + segment.ray.direction * abs(ft);
...
void sort_by_material(int num_paths, ShadeableIntersection * shadeableIntersections, PathSegment * pathSegments) {
	thrust::device_ptr<ShadeableIntersection> dev_intersect_ptr(shadeableIntersections);
	thrust::device_ptr<PathSegment> dev_path_ptr(pathSegments);
	thrust::sort_by_key(thrust::device, dev_intersect_ptr, dev_intersect_ptr + num_paths, dev_path_ptr, MaterialComparator());
}
...
void pathtrace(uchar4 *pbo, int frame, int iter) {
	...
	while (!iterationComplete) {
		...
		if (iter != 1 && depth == 0) {
			...
			checkCUDAError("cudaMemcpy from first bounce intersections");
		}
			...
			cudaDeviceSynchronize();
			cudaMemcpy(dev_firstBounceIntersections, dev_intersections, pixelcount * sizeof(ShadeableIntersection), cudaMemcpyDeviceToDevice);
			checkCUDAError("cudaMemcpy to first bounce intersections");
		}
		else {
			computeIntersections << <numblocksPathSegmentTracing, blockSize1d >> > (
				depth
...
#if SORT_BY_MATERIAL
		sort_by_material(num_paths, dev_intersections, dev_paths);	
#endif
	...
	cout << StreamCompaction::Efficient::timer().getGpuElapsedTimeForPreviousOperation() << endl;
#endif
	//ofstream outFile;
	//outFile.open("data.csv, ios::out");
	//outFile << "iteration" << ',' << iter++ << ',' << "time" << ',' << StreamCompaction::Efficient::timer().getGpuElapsedTimeForPreviousOperation() << endl;
	//outFile.close();
